...
# 256 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino" 2
# 257 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino" 2
# 258 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino" 2
...
long lcdHashLine0 = -3845709 /* Random number that's unlikely to naturally occur as an actual hash*/;
long lcdHashLine1 = -3845709 /* Random number that's unlikely to naturally occur as an actual hash*/;
long lcdHashLine2 = -3845709 /* Random number that's unlikely to naturally occur as an actual hash*/;
long lcdHashLine3 = -3845709 /* Random number that's unlikely to naturally occur as an actual hash*/;
int lastRpm = -1;
...
int measure = 0; // Whether to show distances in inches
int savedMeasure = 0; // measure value saved in Preferences
...
# 1179 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino"
  ...
  server.on("/uploadTft", HTTP_POST,
      ...
      []() {
        ...
        if (upload.status == UPLOAD_FILE_START) {
          ...
          toScreen("whmi-wri " + size + "," + String(115200) + ",0");
  ...
  server.on("/uploadBin", HTTP_POST,
    [&]() {
      ...
      if (Update.hasError()) {
        server.send(200, ((reinterpret_cast<const __FlashStringHelper *>(("text/html")))), String(((reinterpret_cast<const __FlashStringHelper *>(("<META http-equiv=\"refresh\" content=\"5;URL=/\">Update error: "))))) + String(Update.errorString()));
      } else {
        server.client().setNoDelay(true);
        server.send(200, ("text/html"), String(((reinterpret_cast<const __FlashStringHelper *>(("<META http-equiv=\"refresh\" content=\"15;URL=/\">Update Success! Rebooting..."))))));
        delay(100);
        server.client().stop();
        ESP.restart();
      }
    ...
    [&]() {
      ...
      if (upload.status == UPLOAD_FILE_START) {
        ...
        if (upload.name == "filesystem") {
          if (!Update.begin(SPIFFS.totalBytes(), 100)) { //start with max available size
            setText("t2", "upload error");
          }
        } else {
          uint32_t maxSketchSpace = (ESP.getFreeSketchSpace() - 0x1000) & 0xFFFFF000;
          if (!Update.begin(maxSketchSpace, 0)) { //start with max available size
            setText("t2", "upload error");
          }
        }
...
void setEmergencyStop(int kind) {
  ...
  xQueueSemaphoreTake( ( z.mutex ), ( 10 ) );
  xQueueSemaphoreTake( ( x.mutex ), ( 10 ) );
  xQueueSemaphoreTake( ( y.mutex ), ( 10 ) );
}
...
void markAxisOrigin(Axis* a) {
  bool hasSemaphore = xQueueSemaphoreTake( ( a->mutex ), ( 10 ) ) == ( ( BaseType_t ) 1 );
  if (!hasSemaphore) {
    beepFlag = true;
  }
  if (a->leftStop != 0x7fffffffL
# 1344 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino"
                            ) {
    a->leftStop -= a->pos;
  }
  if (a->rightStop != 
# 1347 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino" 3 4
                     (-0x7fffffffL - 1L)
# 1347 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino"
                             ) {
    a->rightStop -= a->pos;
...
String printDupr(long value) {
  if (measure != 2) return printDeciMicrons(value, 5);
...
bool needZStops() {
  return mode == 4 || mode == 5 || mode == 7 || mode == 8;
}
...
bool manualMovesAllowedWhenOn() {
  return mode == 0 || mode == 2 || mode == 3 || mode == 10;
}
...
long getPassModeZStart() {
  if (mode == 4 || mode == 7) return dupr > 0 ? z.rightStop : z.leftStop;
  if (mode == 5) return auxForward ? z.rightStop : z.leftStop;
  if (mode == 8) return dupr > 0 ? z.leftStop : z.rightStop;
  return z.pos;
}

long getPassModeXStart() {
  if (mode == 4 || mode == 7) return auxForward ? x.rightStop : x.leftStop;
  if (mode == 5 || mode == 6) return dupr > 0 ? x.rightStop : x.leftStop;
  if (mode == 8) return x.rightStop;
  return x.pos;
}
...
String printMode() {
  if (mode == 2) return "ASY ";
  if (mode == 3) return "CONE ";
  if (mode == 4) return "TURN ";
  if (mode == 5) return "FACE ";
  if (mode == 6) return "CUT ";
  if (mode == 7) return "THRD ";
  if (mode == 8) return "ELLI ";
  if (mode == 9) return "GCODE ";
  if (mode == 10) return "Y ";
  return "";
}
...
void updateDisplay() {
  ...
  if (splashScreen) {
    ...
    lcdHashLine3 = -3845709 /* Random number that's unlikely to naturally occur as an actual hash*/;
    delay(2000);
  }
  if (lcdHashLine0 == -3845709 /* Random number that's unlikely to naturally occur as an actual hash*/) {
...
# 1707 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino"
                               ) result += "L";
      if (x.leftStop != 0x7fffffffL
# 1708 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino"
                               ) result += "U";
      if (x.rightStop != 
...
bool saveIfChanged() {
  ...
  Preferences pref;
  pref.begin("h5");
  if (dupr != savedDupr) pref.putLong("d", savedDupr = dupr);
  if (starts != savedStarts) pref.putInt("sta", savedStarts = starts);
  if (z.pos != z.savedPos) pref.putLong("zp", z.savedPos = z.pos);
  if (z.posGlobal != z.savedPosGlobal) pref.putLong("zpg", z.savedPosGlobal = z.posGlobal);
  if (z.originPos != z.savedOriginPos) pref.putLong("zpo", z.savedOriginPos = z.originPos);
  if (z.motorPos != z.savedMotorPos) pref.putLong("zpm", z.savedMotorPos = z.motorPos);
  if (z.leftStop != z.savedLeftStop) pref.putLong("zls", z.savedLeftStop = z.leftStop);
  if (z.rightStop != z.savedRightStop) pref.putLong("zrs", z.savedRightStop = z.rightStop);
  if (z.disabled != z.savedDisabled) pref.putBool("zd", z.savedDisabled = z.disabled);
  if (spindlePos != savedSpindlePos) pref.putLong("sp", savedSpindlePos = spindlePos);
  if (spindlePosAvg != savedSpindlePosAvg) pref.putLong("spa", savedSpindlePosAvg = spindlePosAvg);
  if (spindlePosSync != savedSpindlePosSync) pref.putInt("oos", savedSpindlePosSync = spindlePosSync);
  if (spindlePosGlobal != savedSpindlePosGlobal) pref.putLong("spg", savedSpindlePosGlobal = spindlePosGlobal);
  if (showAngle != savedShowAngle) pref.putBool("ang", savedShowAngle = showAngle);
  if (showTacho != savedShowTacho) pref.putBool("rpm", savedShowTacho = showTacho);
  if (moveStep != savedMoveStep) pref.putLong("ms", savedMoveStep = moveStep);
  if (mode != savedMode) pref.putInt("mod", savedMode = mode);
  if (measure != savedMeasure) pref.putInt("mea", savedMeasure = measure);
  if (x.pos != x.savedPos) pref.putLong("xp", x.savedPos = x.pos);
  if (x.posGlobal != x.savedPosGlobal) pref.putLong("xpg", x.savedPosGlobal = x.posGlobal);
  if (x.originPos != x.savedOriginPos) pref.putLong("xpo", x.savedOriginPos = x.originPos);
  if (x.motorPos != x.savedMotorPos) pref.putLong("xpm", x.savedMotorPos = x.motorPos);
  if (x.leftStop != x.savedLeftStop) pref.putLong("xls", x.savedLeftStop = x.leftStop);
  if (x.rightStop != x.savedRightStop) pref.putLong("xrs", x.savedRightStop = x.rightStop);
  if (x.disabled != x.savedDisabled) pref.putBool("xd", x.savedDisabled = x.disabled);
  if (y.pos != y.savedPos) pref.putLong("y1p", y.savedPos = y.pos);
  if (y.posGlobal != y.savedPosGlobal) pref.putLong("y1pg", y.savedPosGlobal = y.posGlobal);
  if (y.originPos != y.savedOriginPos) pref.putLong("y1po", y.savedOriginPos = y.originPos);
  if (y.motorPos != y.savedMotorPos) pref.putLong("y1pm", y.savedMotorPos = y.motorPos);
  if (y.leftStop != y.savedLeftStop) pref.putLong("y1ls", y.savedLeftStop = y.leftStop);
  if (y.rightStop != y.savedRightStop) pref.putLong("y1rs", y.savedRightStop = y.rightStop);
  if (y.disabled != y.savedDisabled) pref.putBool("y1d", y.savedDisabled = y.disabled);
  if (coneRatio != savedConeRatio) pref.putFloat("cr", savedConeRatio = coneRatio);
  if (turnPasses != savedTurnPasses) pref.putInt("tp", savedTurnPasses = turnPasses);
  if (auxForward != savedAuxForward) pref.putBool("af", savedAuxForward = auxForward);
  pref.end();
  return true;
}
...
void setMeasure(int value) {
  ...
  measure = value;
  moveStep = measure == 0 ? 10000 /* 1mm*/ : 25400 /* 1/10"*/;
}
...
bool isContinuousStep() {
  return moveStep == (measure == 0 ? 10000 /* 1mm*/ : 25400 /* 1/10"*/);
}

// For rotational axis the moveStep of 0.1" means 0.1°.
long getMoveStepForAxis(Axis* a) {
  return (a->rotational && measure != 0) ? (moveStep / 25.4) : moveStep;
}
...
bool stepTo(Axis* a, long newPos, bool continuous) {
  if (xQueueSemaphoreTake( ( a->mutex ), ( 10 ) ) == ( ( BaseType_t ) 1 )) {
...
unsigned int getTimerLimit() {
  ...
  return min(long(65535), long(1000000 /* 1MHz async timer frequency*/ / (z.motorSteps * abs(dupr) / z.screwPitch)) - 1);
}
  ...
  while (emergencyStop == 0) {
    ...
    if (isOn && dupr != 0 && mode == 0) {
      ...
      do {
        ...
        if (xQueueSemaphoreTake( ( motionMutex ), ( 100 ) ) == ( ( BaseType_t ) 1 )) {
          if (!resting) {
            spindlePos += diff;
            spindlePosAvg += diff;
          }
...
# 2139 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino"
                        ;
    vPortYield();
  }
  vTaskDelete(
# 2142 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino" 3 4
             __null
# 2142 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino"
                 );
}
...
# 2236 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino"
                        ;
    stepperEnable(&y, false);
    vPortYield();
  }
  vTaskDelete(
# 2240 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino" 3 4
             __null
# 2240 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino"
                 );
}
...
void setFeedRate(const String& command) {
  ...
  if (feed <= 0) return;
  gcodeFeedDuPerSec = round(feed * (measure == 0 ? 10000 : 254000) / 60.0);
}

long mmOrInchToAbsolutePos(Axis* a, float mmOrInch) {
  long scaleToDu = measure == 0 ? 10000 : 254000;
...
void taskGcode(void *param) {
  while (emergencyStop == 0) {
    ...
    if (mode == 9 && isOn && gcodeProgramCharIndex < gcodeProgram.length()) {
    ...
    if (charCode > 0) {
      if (gcodeInBrace) {
        ...
        float divisor = measure == 0 ? 10000.0 : 254000.0;
    ...
    if (mode == 9 && isOn && gcodeProgramCharIndex > 0 && gcodeProgramCharIndex == gcodeProgram.length()) {
      setIsOnFromTask(false);
    }
    vPortYield();
  }
  vTaskDelete(
# 2549 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino" 3 4
             __null
# 2549 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino"
                 );
}
...
void startPulseCounter(pcnt_unit_t unit, int gpioA, int gpioB) {
  ...
  pcntConfig.pos_mode = PCNT_CHANNEL_EDGE_ACTION_INCREASE /*!< Counter mode: Increase counter value */;
  pcntConfig.neg_mode = PCNT_CHANNEL_EDGE_ACTION_HOLD /*!< Counter mode: Inhibit counter(counter value will not change in this condition) */;
  pcntConfig.lctrl_mode = PCNT_CHANNEL_LEVEL_ACTION_KEEP /*!< Control mode: won't change counter mode*/;
  pcntConfig.hctrl_mode = PCNT_CHANNEL_LEVEL_ACTION_INVERSE /*!< Control mode: invert counter mode(increase -> decrease, decrease -> increase) */;
...
void __attribute__((section(".iram1" "." "0"))) onAsyncTimer() {
  Axis* a = getAsyncAxis();
  if (!isOn || a->movingManually || (mode != 2 && mode != 10)) {
    return;
...
# 2790 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino"
    ...
    setEmergencyStop(5);
  } else if (!isOn && on && mode == 9 && gcodeProgramIndex >= gcodeProgramCount) {
    beep();
  } else if (!isOn && on && mode == 9) {
...
void leaveStop(Axis* a, long oldStop) {
  if (mode == 3) {
    ...
    markOrigin();
  } else if (mode == 0 && a == getPitchAxis() && a->pos == oldStop) {
...
void buttonMoveStepPress() {
  if (measure == 0) {
    if (moveStep == 10000 /* 1mm*/) {
      moveStep = 1000 /* 0.1mm*/;
    } else if (moveStep == 1000 /* 0.1mm*/) {
      moveStep = 100 /* 0.01mm*/;
    } else {
      moveStep = 10000 /* 1mm*/;
    }
  } else {
    if (moveStep == 25400 /* 1/10"*/) {
      moveStep = 2540 /* 1/100"*/;
    } else if (moveStep == 2540 /* 1/100"*/) {
      moveStep = 254 /* 1/1000" also known as 1 thou*/;
    } else {
      moveStep = 25400 /* 1/10"*/;
    }
  }
...
bool processNumpadResult(int keyCode) {
  ...
  Axis* a = (keyCode == 65 /* a - sets left stop*/ || keyCode == 68 /* d - sets right stop*/ || keyCode == 21 /* Left arrow - controls Z axis movement to the left*/ || keyCode == 22 /* Right arrow - controls Z axis movement to the right*/ || keyCode == 90 /* z - zeroes Z axis*/) ? &z : &x;
  int sign = ((keyCode == 65 /* a - sets left stop*/ || keyCode == 87 /* w - sets forward stop*/ || keyCode == 21 /* Left arrow - controls Z axis movement to the left*/ || keyCode == 23 /* Up arrow - controls X axis movement forwards*/ || keyCode == 90 /* z - zeroes Z axis*/ || keyCode == 88 /* x - zeroes X axis*/ || keyCode == 67 /* c - enables / disables X axis*/) ? 1 : -1);
  if (mode == 10 && (keyCode == 97 /* F1 - sets the mode to gearbox*/ || keyCode == 98 /* F2 - ...*/ || keyCode == 99 /* F3*/ || keyCode == 100 /* F4*/ || keyCode == 102 /* F6*/)) {
    a = &y;
    sign = (keyCode == 97 /* F1 - sets the mode to gearbox*/ || keyCode == 99 /* F3*/) ? -1 : 1;
  }
  ...
  if (keyCode == 65 /* a - sets left stop*/) {
    setLeftStop(&z, pos);
    return true;
  } else if (keyCode == 68 /* d - sets right stop*/) {
    setRightStop(&z, pos);
    return true;
  } else if (keyCode == 87 /* w - sets forward stop*/) {
    setLeftStop(&x, pos);
    return true;
  } else if (keyCode == 83 /* s - sets rear stop*/) {
    setRightStop(&x, pos);
    return true;
  } else if (mode == 10) {
    if (keyCode == 100 /* F4*/) {
  ...
  if (!isOn && (keyCode == 21 /* Left arrow - controls Z axis movement to the left*/ || keyCode == 22 /* Right arrow - controls Z axis movement to the right*/ || keyCode == 23 /* Up arrow - controls X axis movement forwards*/ || keyCode == 24 /* Down arrow - controls X axis movement backwards*/ || (mode == 10 && (keyCode == 97 /* F1 - sets the mode to gearbox*/ || keyCode == 98 /* F2 - ...*/)))) {
    if (pos < a->rightStop) {
      pos = a->rightStop;
      beep();
  ...
  if (keyCode == 90 /* z - zeroes Z axis*/ || keyCode == 88 /* x - zeroes X axis*/ || (mode == 10 && keyCode == 102 /* F6*/)) {
    a->originPos = -pos;
    return true;
  }

  // Set X axis 0 from diameter.
  if (keyCode == 79 /* o - sets X0 so that centerline is at the middle of a given diameter value*/) {
    a->originPos = -a->pos - posDiffAbs / 2;
    return true;
  }

  if (keyCode == 64 /* Tilda - changes distance moved when movement buttons are used*/) {
    if (newDu > 0) {
      moveStep = newDu;
    } else {
      beep();
...
bool processNumpad(int keyCode) {
  if (keyCode == 48 /* 0 top row - for number entry*/) {
    numpadPress(0);
    inNumpad = true;
  } else if (keyCode == 49 /* 1 top row*/) {
    numpadPress(1);
    inNumpad = true;
  } else if (keyCode == 50 /* ...*/) {
    numpadPress(2);
    ...
    numpadPress(9);
    inNumpad = true;
  } else if (keyCode == 28 /* removes the last entered number*/) {
    numpadBackspace();
    inNumpad = true;
  } else if (inNumpad && (keyCode == 44 /* Numpad plus - increments the pitch or number of passes*/ || keyCode == 45 /* Numpad minus - recrements the pitch or number of passes*/)) {
    numpadPlusMinus(keyCode == 44 /* Numpad plus - increments the pitch or number of passes*/);
    return true;
  } else if (inNumpad) {
    inNumpad = false;
...
void processKeypadEvent() {
  ...
  if (keyCode == 27 /* ESC - stops operation or mode*/) {
  ...
  if (mode == 9 && isOn) {
    ...
    if (isPress && keyCode != 27 /* ESC - stops operation or mode*/) beep();
    return;
  }
  ...
  if (isPress && setupIndex == 2 && (keyCode == 21 /* Left arrow - controls Z axis movement to the left*/ || keyCode == 22 /* Right arrow - controls Z axis movement to the right*/)) {
    auxForward = !auxForward;
  } else if (isPress && mode == 9 && setupIndex == 1 && (keyCode == 23 /* Up arrow - controls X axis movement forwards*/ || keyCode == 24 /* Down arrow - controls X axis movement backwards*/)) {
    if (gcodeProgramIndex > 0 && keyCode == 23 /* Up arrow - controls X axis movement forwards*/) gcodeProgramIndex--;
    else if (gcodeProgramIndex == 0 && gcodeProgramCount > 0 && keyCode == 23 /* Up arrow - controls X axis movement forwards*/) gcodeProgramIndex = gcodeProgramCount - 1;
    else if ((gcodeProgramIndex < gcodeProgramCount - 1) && keyCode == 24 /* Down arrow - controls X axis movement backwards*/) gcodeProgramIndex++;
    else if (keyCode == 24 /* Down arrow - controls X axis movement backwards*/) gcodeProgramIndex = 0;
  } else if (isPress && mode == 9 && setupIndex == 1 && keyCode == 45 /* Numpad minus - recrements the pitch or number of passes*/) {
    removeGcodeByName(getCurrentGcodeProgramName());
    return;
  } else if (keyCode == 21 /* Left arrow - controls Z axis movement to the left*/) { // Make sure isPress=false propagates to motion flags.
    buttonLeftPressed = isPress;
  } else if (keyCode == 22 /* Right arrow - controls Z axis movement to the right*/) {
    buttonRightPressed = isPress;
  } else if (keyCode == 23 /* Up arrow - controls X axis movement forwards*/) {
    buttonUpPressed = isPress;
  } else if (keyCode == 24 /* Down arrow - controls X axis movement backwards*/) {
    buttonDownPressed = isPress;
  } else if (keyCode == 97 /* F1 - sets the mode to gearbox*/) {
  ...
  // Rest of the buttons.
  if (keyCode == 44 /* Numpad plus - increments the pitch or number of passes*/) {
    buttonPlusMinusPress(true);
  } else if (keyCode == 45 /* Numpad minus - recrements the pitch or number of passes*/) {
    buttonPlusMinusPress(false);
  } else if (keyCode == 30 /* Enter - starts operation or mode*/) {
    buttonOnOffPress(true);
  } else if (keyCode == 65 /* a - sets left stop*/) {
  ...
  } else if (keyCode == 106 /* F10*/) {
    if (ACTIVE_Y) setModeFromTask(10);
  } else if (keyCode == 104 /* F8*/) {
    setModeFromTask(8);
  } else if (keyCode == 105 /* F9*/) {
    setModeFromTask(9);
  } else if (keyCode == 103 /* F7*/) {
    setModeFromTask(2);
  } else if (keyCode == 12 /* Win - changes info displayed in the bottom line (angle, rpm, ...)*/) {
    buttonDisplayPress();
  } else if (keyCode == 88 /* x - zeroes X axis*/) {
    markAxis0(&x);
  } else if (keyCode == 90 /* z - zeroes Z axis*/) {
    markAxis0(&z);
  } else if (keyCode == 67 /* c - enables / disables X axis*/) {
    x.disabled = !x.disabled;
    updateEnable(&x);
  } else if (keyCode == 81 /* q - enables / disables Z axis*/) {
    z.disabled = !z.disabled;
    updateEnable(&z);
  } else if (keyCode == 64 /* Tilda - changes distance moved when movement buttons are used*/) {
    buttonMoveStepPress();
  } else if (keyCode == 14 /* Context menu - not used currently*/) {
    // TODO.
  } else if (keyCode == 82 /* r - changes pitch sign (left / right thread)*/) {
    buttonReversePress();
  } else if (keyCode == 77 /* m - controls metric / imperial / tpi*/) {
    buttonMeasurePress();
  } else if (keyCode == 97 /* F1 - sets the mode to gearbox*/ && mode != 10) {
    setModeFromTask(0);
  } else if (keyCode == 98 /* F2 - ...*/ && mode != 10) {
    setModeFromTask(4);
  } else if (keyCode == 99 /* F3*/) {
    mode == 10 ? buttonRightStopPress(&y) : setModeFromTask(5);
  } else if (keyCode == 100 /* F4*/) {
    mode == 10 ? buttonLeftStopPress(&y) : setModeFromTask(3);
    ...
    mode == 10 || (mode == 9 && ACTIVE_Y) ? markAxis0(&y) : setModeFromTask(7);
  }
}
...
void moveAxis(Axis* a) {
  ...
  if (xQueueSemaphoreTake( ( a->mutex ), ( 1 ) ) == ( ( BaseType_t ) 1 )) {
...
# 3509 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino"
  ...
  if (opIndex == 0) {
    ...
    if (opSubIndex == 0) {
      ...
      float mainCoeff = auxForward ? cos(1.5707963267948966192313216916398 * (3 + progress0to1)) : (1 + sin(1.5707963267948966192313216916398 * (progress0to1 - 1)));
      long mainPos = mainEndStop - mainDelta + round(mainDelta * mainCoeff);
      float auxCoeff = auxForward ? (1 + sin(1.5707963267948966192313216916398 * (3 + progress0to1))) : sin(1.5707963267948966192313216916398 * progress0to1);
...
void discountFullSpindleTurns() {
  ...
  if (dupr != 0 && !stepperIsRunning(&z) && (mode == 0 || mode == 3)) {
  ...
  if (Serial1.available()) {
    ...
    if (bytesRead >= 2) {
      ...
      case 0x97:
        ...
        break;
      case 0x98:
        processNumpad(30 /* Enter - starts operation or mode*/);
        break;
      case 0x99:
...
void setup() {
  ...
  motionMutex = xQueueCreateMutex( ( ( uint8_t ) 1U ) );
  savedStarts = starts = min(STARTS_MAX, max(static_cast<int32_t>(1), pref.getInt("sta")));
  z.savedPos = z.pos = pref.getLong("zp");
  z.savedPosGlobal = z.posGlobal = pref.getLong("zpg");
...
# 3841 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino"
  ...
  y.savedPosGlobal = y.posGlobal = pref.getLong("y1pg");
...
# 3848 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino"
  ...
  savedSpindlePosAvg = spindlePosAvg = pref.getLong("spa");
  savedSpindlePosSync = spindlePosSync = pref.getInt("oos");
  savedSpindlePosGlobal = spindlePosGlobal = pref.getLong("spg");
  savedShowAngle = showAngle = pref.getBool("ang");
  savedShowTacho = showTacho = pref.getBool("rpm");
  savedMoveStep = moveStep = pref.getLong("ms", 10000 /* 1mm*/);
  setModeFromLoop(savedMode = pref.getInt("mod"));
  savedMeasure = measure = pref.getInt("mea");
  savedConeRatio = coneRatio = pref.getFloat("cr", coneRatio);
  savedTurnPasses = turnPasses = pref.getInt("tp", turnPasses);
  ...
  if (KEYBOARD_ENABLED) {
    keyboard.begin(37, 36);
    xTaskCreatePinnedToCore(taskKeypad, "taskKeypad", 10000 /* stack size */, 
# 3881 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino" 3 4
                                                                             __null
# 3881 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino"
                                                                                 , 0 /* priority */, 
# 3881 "C:\\Users\\Andre\\Documents\\nanoels\\h5\\h5.ino" 3 4
...
void loop() {
  ...
  if (xQueueSemaphoreTake( ( motionMutex ), ( 1 ) ) != ( ( BaseType_t ) 1 )) {
    return;
  }
  applySettings();
